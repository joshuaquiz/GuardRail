...
#include "../../include/nfc/nfc-emulation.h"

#include "../../include/nfc/iso7816.h"
...
{
    uint8_t abt_rx[ISO7816_SHORT_R_APDU_MAX_LEN];
    uint8_t abt_tx[ISO7816_SHORT_C_APDU_MAX_LEN];

    int res;
    if ((res = nfc_target_init(pnd, emulator->target, abt_rx, sizeof abt_rx, timeout)) < 0)
    {
        return res;
    }
    ...
    while (io_res >= 0)
    {
        io_res = emulator->state_machine->io(emulator, abt_rx, sz_rx, abt_tx, sizeof abt_tx);
        if (io_res > 0)
        {
            if ((res = nfc_target_send_bytes(pnd, abt_tx, io_res, timeout)) < 0)
            {
                return res;
            }
        }
        if (io_res >= 0)
        {
            if ((res = nfc_target_receive_bytes(pnd, abt_rx, sizeof abt_rx, timeout)) < 0)
            {
                return res;
            }
