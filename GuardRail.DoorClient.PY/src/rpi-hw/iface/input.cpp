...
#include "../../../include/rpi-hw/iface/input.hpp"

namespace rpihw { // Begin main namespace

namespace iface { // Begin interfaces namespace

input::input( std::initializer_list< uint8_t > pins, uint8_t pull_mode ) : iface::iface_base( pins ), m_pull_mode( pull_mode ) {
...
input::input( const std::vector< uint8_t > &pins, uint8_t pull_mode ) : iface::iface_base( pins ), m_pull_mode( pull_mode ) {
...
size_t input::read() const {

	// Read data from the interface
	size_t data = 0, i = 0;

	for ( ; i < size(); ++i )
		data |= static_cast<size_t>(m_gpio->read(m_pins[i])) << i;
