...
designer< T, C, N >::drawEllipse( T cx, T cy, T a, T b ) {
	...
	d = math::round( static_cast<float>(bb) * math::square( x + 0.5 ) + aa * math::square( y - 1 ) - aa * bb );
...
designer< T, C, N >::fillRect( T x0, T y0, T x1, T y1 ) {
	...
	for ( ; i < total; ++i )
		drawPoint( static_cast<int>(x0 + i % w), static_cast<int>(y0 + i / w) );
}
...
designer< T, C, N >::drawChar( char32_t charcode ) {
	...
	if ( glyph.pitch > 0 ) {
		...
		pitch	= static_cast<uint8_t>(glyph.pitch);
		step	= 1;
...
designer< T, C, N >::drawText( iterator it, iterator end ) {
	...
	for ( ; it != end; ++it ) {
		...
		if ( *it != '\n' )
			drawText( static_cast<char32_t>(*it) );
	...
	if ( word_break ) {
		...
		for ( ; it != end; ++it ) {
			...
			if ( *it == '\n' ) {
				...
				for ( ; off != it; ++off )
					drawChar( static_cast<char32_t>(*off) );
